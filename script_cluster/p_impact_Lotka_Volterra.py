...
name_csv= "lotka_volterra_dim_%s_p"%A.shape[0]

l_diffusion_strength = None#np.geomspace(model.diffusion_strength/100, model.diffusion_strength*10, num_dot)
l_n = np.geomspace(model.n/1000, model.n, num_dot).astype(int)
l_dt = None#np.round(np.geomspace(model.dt, model.dt*100, num_dot) / model.dt) * model.dt
...
if __name__ == "__main__":
    kwargs = {"name_csv" : name_csv, "l_diffusion_strength" : l_diffusion_strength, "l_n" : l_n, "l_dt" : l_dt,
              "l_experimental_noise" : l_experimental_noise}
    ...
    para_inference = InferenceParameter(model, use_sindy=False, use_PASTIS=True, use_AIC=False, use_BIC=False, use_CrossValidation=False)
