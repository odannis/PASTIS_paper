...
l_dt = np.geomspace(model.dt, model.dt*100, num_dot) 
l_experimental_noise = np.geomspace(0.001, 1, num_dot) 
l_thresholds_sindy = None #np.unique(np.round(np.linspace(0.01, 1, num_dot), 2))

para_inference_critical = InferenceParameter(model, use_sindy=False, use_PASTIS=True, use_AIC=True, use_CrossValidation=True, use_BIC=True) #use_PASTIS=True, use_AIC=True, use_CrossValidation=True, use_BIC=True
